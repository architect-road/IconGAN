...
class AugmentPipe(torch.nn.Module):
    def __init__(self,
        xflip=0, rotate90=0, xint=0, xint_max=0.1,
        scale=0, rotate=0, aniso=0, xfrac=0, scale_std=0.1, rotate_max=1, aniso_std=0.2, xfrac_std=0.125,
    ...
    def forward(self, images, debug_percentile=None):
        ...
        if C is not I_4:
            ...
            elif num_channels == 4: # for RGBA or RGBa, do not change alpha channel
                images = torch.cat((C[:, :3, :3] @ images[:,0:3,:] + C[:, :3, 3:], images[:,3:,:]), dim=1)
            else:
